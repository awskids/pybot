...
#########################################################################
# Details
# *. Scan point for all vectors
# *. - rotate in place, sampling colors.
# *. - store each angle that has target color
# *. identify possible paths (mid-points) in contiguous found color
# *. pick direction (STRATEGY#1: closest to current driving vector)
...
def scan_in_place(greedy = False):
    # TODO: ensure oriented same at end as start (current_vector = 0)
    vectors = []
    ...
    while current_vector <= 360:
        ...
        # TODO: do we need to wait for the sensor?
        utime.sleep(0.1)
...
def pick_direction__closest_driving_vector(vectors):
    if len(vectors) > 0:
        vectors.sort(key = sortSecond)
        # print(vectors)
        vector = vectors[0] # closest
        return vector[0]
    else:
        return None
...
def move_until_offpath(start_vector):
    ...
    # TODO: convert start_vector into a steeringunit?
    turn(start_vector)
...
def deep_scan():
    vectors = scan_in_place(False)
    vector = pick_direction__closest_driving_vector(vectors)

    print('deep_scan', 'found vectors', vectors)
    print('deep_scan', 'selected vector', vector)
    return vector
